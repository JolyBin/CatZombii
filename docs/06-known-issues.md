...
| 5 | Успех и провал комбинации выглядят одинаково | ❌ **открыт**, приоритет ↑ | — |
| 6 | `RepitsCommand.Invoke` без null-проверки | ✅ починен | `d162c4db` |
| 7 | Стан вместо паузы жжёт CPU | ✅ починен | `d162c4db` |
...
| 12 | Пазл остаётся интерактивным под окном поражения | ⚠️ **починен наполовину** | `d162c4db` |
...
Открытыми остаются **№5, №9, №10, №16** и половина **№12**. Всё остальное закрыто
и проверено по коду.
...
**Статус: ❌ открыт, приоритет повышен 01.08.2026.** `ShowResult` по-прежнему игнорирует
`result`. Повышение приоритета: с часами котла ([10 §1](10-progression-design.md), 8 с)
провал перестаёт быть бесплатным — он стоит игроку накопленной цепочки **и** времени,
которое тикало на кольце. Невидимый провал в такой конструкции ломает главную обратную
связь ядра, поэтому фидбек идёт в И1 вместе с котлом, а не откладывается в подачу
...
**Статус: ✅ починен в `d162c4db`.** Шаг таймера стал константой `TIMER_STEP = 100`,
пауза — отдельным флагом `_isPaused`. `UniTask.Delay` всегда 100 мс. Сверх того
цикл получил токен боя и завершается по отмене, а не крутится до `Dispose`.
...
**Статус: ⚠️ починен наполовину в `d162c4db`.**

Что закрыто: бой действительно останавливается. `EndBattle()` гасит таймеры обеих
сторон и отменяет токен боя, поэтому циклы `TimerAttack` завершаются, а не крутятся
до нажатия «Продолжить». Умножение [бага №7](#7--стан-вместо-паузы-жжёт-cpu) на число
врагов исчезло вместе с самим багом №7.

Что осталось открытым: **пазл под окнами победы и поражения по-прежнему живой**.
`ShowWinWindow` / `ShowLoseWindow` только показывают окно; `FlaskController`
и `TableController` не трогаются до `Exit()`, поэтому игрок продолжает переливать
шарики, копить элементы и жать «проверить». Заклинания при этом отрабатывают
частично: спавн союзников и врагов отсекается флагом `_isBattleOver`, а прямой
урон по ещё живым врагам проходит. Паузы боя в игре так и не существует —
она всё равно нужна для рекламы и для сворачивания мобильного браузера
