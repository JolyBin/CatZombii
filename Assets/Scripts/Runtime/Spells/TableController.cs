...
namespace Core.Spells
{
    ...
    public class TableController
    {
        ...
        ///
        /// Порядок вставки здесь — это ПОРЯДОК ГИБЕЛИ, и на этом стоит вся слотовая
        /// индикация (docs/12 §5.2). Очередь снимается только с головы, каждое снятие
        /// уносит ХВОСТ котла (<see cref="DropTail"/>), значит k-й срок в очереди — это
        /// ровно k-е удаление, то есть k-й элемент с конца. Отсюда правило отрисовки:
        /// ближайший срок рисуется на ХВОСТОВОМ слоте, следующий — на предпоследнем,
        /// и так к голове; голова котла живёт дольше всех и гаснет последней.
        ///
        /// Часы монотонны (<see cref="_clockTime"/> только растёт), поэтому порядок
        /// очереди совпадает с возрастанием сроков. Виду отдаётся именно порядок очереди,
        /// а не отсортированный список: порядок очереди — это порядок снятия по факту,
        /// он остаётся верным даже если ручку <see cref="UITableWindow.CoolingSeconds"/>
        /// крутанут вживую посреди боя.
        /// </summary>
        private Queue<float> _coolingDeadlines;

        /// <summary>
        /// Буфер остатков для вида. Переиспользуется: часы толкают вид каждые 100 мс,
        /// и новый список на каждый тик — это мусор в GC на мобильном WebGL.
        /// </summary>
        private readonly List<float> _coolingRemaining = new();
        ...
        /// <summary>
        /// Отдаёт виду ВСЕ оставшиеся сроки, а не только ближайший: у каждого элемента
        /// в котле своё кольцо, и одно число на всех больше ничего не объясняет —
        /// именно оно и путало («всё сейчас слетит или один?»), решение владельца
        /// от 01.08.2026 (docs/12 §5.2).
        ///
        /// Порядок списка — порядок гибели: [0] уйдёт первым. Раскладку по слотам
        /// делает вид, потому что только он знает, сколько слотов занято.
        /// </summary>
        private void PushCoolingToView()
        {
            ...
            _coolingRemaining.Clear();
            // обход Queue<float> идёт от головы к хвосту — это и есть порядок снятия
            foreach (float deadline in _coolingDeadlines)
                _coolingRemaining.Add(deadline - _clockTime);

            _window.SetCooling(_coolingRemaining, _window.CoolingSeconds);
        }
